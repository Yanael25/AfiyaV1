...
export function Home() {
  ...
  return (
    <div className="bg-[#FAFAF8] min-h-screen flex flex-col pb-[100px] font-sans">
      
      {/* HEADER */}
      <div className="pt-[52px] px-[24px] pb-[16px] flex justify-between items-start">
        <div>
          ...
          <div className="flex items-center gap-[7px]">
            ...
            <div className="text-[12px] font-bold text-[#047857]">{profile?.score_afiya || 0} pts</div>
            <div className="w-[3px] h-[3px] bg-[#C4B8AC] rounded-full" />
        ...
        <div 
          className="relative w-[42px] h-[42px] rounded-[14px] bg-[#047857] flex items-center justify-center text-[13px] font-extrabold text-white shrink-0 mt-[4px] cursor-pointer transition-opacity active:opacity-80" 
      ...
      <div 
        ...
        className="pl-[24px] overflow-x-auto flex gap-[12px] mb-[10px] snap-x snap-mandatory scrollbar-hide"
        style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
      >
        {/* Card Principal */}
        <div className="min-w-[292px] w-[292px] bg-white rounded-[24px] p-[22px] shrink-0 flex flex-col snap-center cursor-pointer transition-transform active:scale-[0.98]">
          ...
          <div className="text-[34px] font-extrabold text-[#1A1A1A] tracking-[-0.03em] leading-none mb-[3px]">
            {balance === null ? '...' : formatXOF(balance).replace(' FCFA', '')}
          </div>
          <div className="text-[12px] font-medium text-[#A39887] mb-[16px]">FCFA disponibles</div>
          <div className="bg-[#FAFAF8] rounded-[14px] p-[10px] px-[14px] h-[52px] flex flex-col justify-center">
        ...
        <div className="min-w-[292px] w-[292px] bg-white rounded-[24px] p-[22px] shrink-0 flex flex-col snap-center cursor-pointer transition-transform active:scale-[0.98]">
          ...
          <div className="text-[34px] font-extrabold text-[#1A1A1A] tracking-[-0.03em] leading-none mb-[3px]">
            {balanceCercles === null ? '...' : formatXOF(balanceCercles).replace(' FCFA', '')}
          </div>
          <div className="text-[12px] font-medium text-[#A39887] mb-[16px]">FCFA disponibles</div>
          <div className="bg-[#FAFAF8] rounded-[14px] p-[10px] px-[14px] h-[52px] flex flex-col justify-center">
            <div className="text-[10px] font-bold tracking-[0.1em] uppercase text-[#C4B8AC] mb-[5px]">A venir</div>
            {cycleInfo ? (
              <div className={`text-[15px] font-extrabold ${cycleInfo.type === 'À recevoir' ? 'text-[#047857]' : 'text-[#1A1A1A]'}`}>
                {cycleInfo.type === 'À recevoir' ? '+' : '-'}{formatXOF(cycleInfo.amount)}
              </div>
            ) : (
              <div className="text-[11px] font-normal text-[#C4B8AC] italic">Aucun mouvement prévu</div>
            )}
        ...
        {/* Card Capital */}
        <div className="min-w-[292px] w-[292px] bg-white rounded-[24px] p-[22px] shrink-0 flex flex-col snap-center mr-[24px] cursor-pointer transition-transform active:scale-[0.98]">
          <div className="text-[11px] font-bold tracking-[0.1em] uppercase text-[#A39887] mb-[14px]">Afiya Capital</div>
          <div className="text-[34px] font-extrabold text-[#1A1A1A] tracking-[-0.03em] leading-none mb-[3px]">
            {balanceCapital === null ? '...' : formatXOF(balanceCapital).replace(' FCFA', '')}
          </div>
      ...
      <div className="flex justify-center gap-[5px] mb-[24px] mt-[4px]">
        {[0, 1, 2].map((i) => (
          <div 
            key={i} 
      ...
      {/* ACTIONS GLOBALES */}
      <div className="mx-[24px] mb-[28px] grid grid-cols-3 gap-[10px]">
        <motion.button whileTap={{ scale: 0.95 }} className="bg-white rounded-[18px] py-[14px] flex flex-col items-center gap-[8px]">
        ...
        <motion.button whileTap={{ scale: 0.95 }} className="bg-white rounded-[18px] py-[14px] flex flex-col items-center gap-[8px]">
          <div className="w-[36px] h-[36px] rounded-[12px] bg-[#F0FDF4] flex items-center justify-center">
            <QrCode size={18} strokeWidth={1.5} className="text-[#047857]" />
          </div>
      ...
      <div className="mx-[24px]">
        <div className="flex justify-between items-center mb-[12px] px-[2px]">
          <div className="text-[14px] font-extrabold text-[#1A1A1A]">Transactions récentes</div>
          <div className="text-[12px] font-semibold text-[#047857] cursor-pointer transition-opacity active:opacity-80">Voir tout</div>
        </div>
                ...
                <div className="flex items-center gap-[12px] px-[16px] py-[14px]">
                  <div className={`w-[36px] h-[36px] rounded-[12px] shrink-0 flex items-center justify-center ${isCredit(tx.type) ? 'bg-[#F0FDF4]' : 'bg-[#F5F4F2]'}`}>
                    {isCredit(tx.type) ? (
                      ...
                      <ArrowUp size={18} strokeWidth={1.5} className="text-[#6B6B6B]" />
                    )}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-[#1A1A1A] mb-[2px] truncate leading-tight">{tx.description || tx.type}</div>
                    <div className="text-[11px] font-normal text-[#A39887] leading-tight">{formatDate(tx.created_at)}</div>
                  </div>
                  <div className={`text-[14px] font-extrabold whitespace-nowrap shrink-0 ${isCredit(tx.type) ? 'text-[#047857]' : 'text-[#1A1A1A]'}`}>
                    {isCredit(tx.type) ? '+' : '-'}{formatXOF(tx.amount).replace(' FCFA', '')}
                  </div>
                </div>
