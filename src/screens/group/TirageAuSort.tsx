...
export function TirageAuSort() {
  ...
  const loadData = async () => {
    try {
      ...
      // Sort members by draw_position (nulls at the end)
  ...
  const handleRefuse = async () => {
    ...
    try {
      ...
      setError(e.message || "Une erreur est survenue lors du repositionnement");
      setLoading(false);
    }
  };
  ...
  // Calculate differential for display
  const adjusted_deposit = myMember.initial_deposit * 2.0;
  const deposit_differential = adjusted_deposit - myMember.initial_deposit;

  return (
    <div className="min-h-screen bg-[#FAFAF8] font-manrope flex flex-col overflow-y-auto pb-10">
      {/* TOP BAR */}
      <div className="pt-[52px] px-6 mb-6 flex items-center gap-3">
        <button 
          onClick={() => navigate(-1)}
        ...
        <div>
          ...
          <div className="text-[24px] font-extrabold text-[#1A1A1A] tracking-tight leading-tight">
            {isDrawn ? "Le tirage a eu lieu." : "Tirage des positions"}
          </div>
        </div>
      </div>
      ...
      {!isDrawn && (
        <div className="flex flex-col items-center px-8 text-center mt-8 gap-4">
          <div className="w-16 h-16 bg-[#F0FDF4] rounded-full flex items-center justify-center mx-auto">
            <Clock size={28} stroke="#047857" strokeWidth={1.5} />
          </div>
          ...
          <div className="bg-[#FAFAF8] rounded-[14px] p-3.5 px-4 w-full mt-4">
          ...
          <button 
            onClick={() => navigate(-1)} 
            className="w-full bg-[#047857] text-white rounded-[16px] py-4 text-[15px] font-bold mx-auto mt-8"
          >
            Retour au cercle
          </button>
        </div>
      ...
      {isDrawn && (
        <>
          ...
          <div className="flex flex-col items-center mb-7">
            <div className="text-[12px] font-semibold text-[#A39887] mb-4 tracking-[0.02em]">Votre position de tirage</div>
            ...
            <div className="text-[15px] font-bold text-[#1A1A1A] mb-1.5 mt-5">
              Position {myMember.draw_position} sur {group.target_members}
            </div>
            
            <div className="text-[13px] font-normal text-[#A39887] text-center leading-relaxed max-w-[240px]">
              {isCaseC 
                ? `Vous recevrez la cagnotte au ${myMember.draw_position}${myMember.draw_position === 1 ? 'er' : 'ème'} cycle. Un complément de caution est requis.` 
                : `Vous recevrez la cagnotte au ${myMember.draw_position}${myMember.draw_position === 1 ? 'er' : 'ème'} cycle. Aucune action requise.`
              }
            </div>
          </div>
          ...
          {isCaseC && !myMember.deposit_differential_paid && (
            <div className="bg-[#F0FDF4] rounded-[14px] mx-4 mb-4 p-3.5 flex items-start gap-2">
              ...
              <div className="text-[12px] font-medium text-[#047857] leading-relaxed">
                Un complément de {formatXOF(deposit_differential)} est requis sous 48h. Sans régularisation, votre position sera automatiquement déplacée dans la seconde moitié du groupe, sans pénalité.
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-[20px] p-4 px-[18px] mx-4 mb-5">
            <div className="uppercase text-[11px] font-bold text-[#A39887] mb-3">ORDRE DE RÉCEPTION</div>
            
            {members.map((m) => {
              const isMe = m.user_id === auth.currentUser?.uid;
              const isTop30 = m.draw_position !== null && m.draw_position <= top30Threshold;
              const needsDiff = isTop30 && !m.deposit_differential_paid;
              
              return (
                <div key={m.id} className="flex items-center gap-3 py-2.5 border-b border-[#F8F7F6] last:border-0">
                  <div className={`w-7 h-7 rounded-[9px] flex items-center justify-center flex-shrink-0 text-[12px] font-extrabold ${isMe ? 'bg-[#047857] text-white' : 'bg-[#FAFAF8] text-[#A39887]'}`}>
                    {m.draw_position || '-'}
                  </div>
                  
                  <div className="flex-1">
                    <div className="text-[13px] font-semibold text-[#1A1A1A] mb-0.5">
                      {m.member_name || 'Membre'}
                    </div>
                    <div className="text-[11px] text-[#A39887]">
                      Cycle {m.draw_position || '?'}
                    </div>
                  </div>
                  
                  <div>
                    {isMe ? (
                      <div className={`text-[10px] font-bold px-2.5 py-1 rounded-[8px] ${needsDiff ? 'bg-[#F5F4F2] text-[#6B6B6B]' : 'bg-[#F0FDF4] text-[#047857]'}`}>
                        {needsDiff ? 'Moi · À compléter' : 'Moi'}
                      </div>
                    ) : needsDiff ? (
                      <div className="bg-[#F5F4F2] text-[#6B6B6B] text-[10px] font-bold px-2.5 py-1 rounded-[8px]">
                        Complément requis
                      </div>
                    ) : (
                      <div className="bg-[#F5F4F2] text-[#A39887] text-[10px] font-bold px-2.5 py-1 rounded-[8px]">
                        —
                      </div>
                    )}
                  </div>
          ...
          {/* CTAs */}
          {isCaseC && !myMember.deposit_differential_paid ? (
            <div className="mx-4 flex flex-col gap-2.5 mt-2">
              <button 
                onClick={() => navigate(`/group/${groupId}/adjust-deposit`)} 
                className="w-full bg-[#047857] text-white rounded-[16px] py-4 text-[15px] font-bold"
              >
